...
function Headphone() {
  ...
  const [headphone, setHeadphone] = useState<headphone[]>([]);
  const [earbuds, setEarbuds] = useState([]);
  const [pickColorPro, setPickColorPro] = useState<string>("black");
  const [colorCode, setColorCode] = useState<string>("00000");
  ...
  const getData = async () => {
    try {
      const headphone = (await axios.get(`${API}/headphone`)).data;
      const earbuds = (await axios.get(`${API}/earbuds`)).data;
  ...
  const addToCart = async (detail: headphone) => {
    try {
  ...
  return (
    <>
      ...
      {headphone.length !== 0 ? (
        <>
          <section
            className={`flex flex-col justify-start items-center bg-red-500 w-full `}
          >
            ...
            {windowWidth < 1024 ? (
              <>
                <div
                  className={
                    "bg-white w-11/12 max-w-lg h-full mt-3  rounded-lg flex flex-col mb-5 p-3 justify-center items-center "
                  }
                >
                        ...
                        {selectProduct === product.name ? (
                          <>
                            <div className="flex flex-col justify-center items-center text-center">
                              ...
                              <div className="bg-red-500 p-2 rounded-lg gap-2 grid grid-cols-2 font-bold  text-center">
        ...
        <>
          <div className="h-full flex justify-center items-center bg-red-500 p-5">
